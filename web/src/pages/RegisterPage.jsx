...
export default function RegisterPage() {
  ...
  // detect mobile for performance (reduce particles)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let pid = 0;
    const interval = setInterval(() => {
      if (isMobile && particles.length > 12) return; // limit on mobile
      const p = {
        id: pid++,
        x: Math.random() * 100,
        y: Math.random() * 100,
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: '#03080f', minHeight: '100vh', position: 'relative' }}>
      ...
      {/* Aurora orbs – hidden on very small screens to save performance */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
        {!isMobile && (
          <>
            <div style={{ position:'fixed', width:700, height:700, borderRadius:'50%', background:'radial-gradient(circle,#7c3aed,#312e81)', top:'-200px', left:'-200px', filter:'blur(80px)', opacity:.17, pointerEvents:'none', zIndex:0, animation:'float1 18s ease-in-out infinite' }} />
            <div style={{ position:'fixed', width:600, height:600, borderRadius:'50%', background:'radial-gradient(circle,#06b6d4,#0c4a6e)', bottom:'-100px', left:'20%', filter:'blur(80px)', opacity:.17, pointerEvents:'none', zIndex:0, animation:'float2 22s ease-in-out infinite' }} />
            <div style={{ position:'fixed', width:500, height:500, borderRadius:'50%', background:'radial-gradient(circle,#10b981,#064e3b)', top:'30%', right:'-100px', filter:'blur(80px)', opacity:.17, pointerEvents:'none', zIndex:0, animation:'float3 16s ease-in-out infinite' }} />
            <div style={{ position:'fixed', width:400, height:400, borderRadius:'50%', background:'radial-gradient(circle,#f43f5e,#7c3aed)', bottom:'10%', right:'5%', filter:'blur(80px)', opacity:.17, pointerEvents:'none', zIndex:0, animation:'float4 20s ease-in-out infinite' }} />
          </>
        )}
      </div>

      {/* Grid mesh */}
      <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0, backgroundImage:'linear-gradient(rgba(255,255,255,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.03) 1px,transparent 1px)', backgroundSize:'50px 50px' }} />

      {/* Particles */}
      <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:1, overflow:'hidden' }}>
        {particles.map(p => (
          <div key={p.id} style={{ position:'absolute', width:p.size, height:p.size, borderRadius:'50%', background:p.color, left:`${p.x}%`, bottom:`${p.y}%`, animation:`particleFly ${p.dur}s ease-in forwards` }} />
        ))}
      </div>
      ...
      <div className="register-grid" style={{
        ...
        <div style={{
          display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
          padding: 'clamp(1.5rem, 5vw, 3rem) clamp(1.5rem, 5vw, 3rem)',
          opacity: mounted ? 1 : 0, transition: 'opacity .6s ease',
        }}>
          <div style={{
            opacity: mounted ? 1 : 0,
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.75rem' }}>
              <div style={{ width: 90, height: 90, position: 'relative' }}>
                <div style={{ position: 'absolute', inset: -2, background: 'conic-gradient(from 0deg,#7c3aed,#06b6d4,#10b981,#f43f5e,#7c3aed)', borderRadius: 18, animation: 'spinBorder 4s linear infinite' }} />
                <div style={{ position: 'absolute', inset: 2, background: '#0a0f1e', borderRadius: 13, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <img src="/logo.png" alt="Logo" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                </div>
              </div>
              <div>
                <div style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 800, fontSize: 'clamp(0.9rem, 4vw, 1.125rem)', color: '#fff', letterSpacing: '-.03em', lineHeight: 1.1 }}>MR Techlab LLP</div>
                <div style={{ fontSize: 'clamp(0.5rem, 3vw, 0.625rem)', color: 'rgba(255,255,255,0.3)', letterSpacing: '.15em', textTransform: 'uppercase', fontWeight: 600 }}>Internship Platform</div>
              </div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', padding: '.3rem .875rem', borderRadius: 999, border: '1px solid rgba(16,185,129,0.3)', background: 'rgba(16,185,129,0.08)', marginBottom: '1.5rem' }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#10b981', animation: 'pulseGlow 2s ease infinite' }} />
              <span style={{ fontSize: '.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.6)', letterSpacing: '.04em' }}>Now accepting applications</span>
            </div>
            ...
            <h1 style={{
              ...
              lineHeight: 1.1, letterSpacing: '-.04em', color: '#fff', marginBottom: '1.25rem',
            }}>
              Start your<br />internship<br />
              <span style={{ background: 'linear-gradient(135deg,#a78bfa 0%,#67e8f9 50%,#6ee7b7 100%)', backgroundSize: '200% 200%', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', animation: 'gradShift 5s ease infinite' }}>
                journey today.
              </span>
            </h1>

            <p style={{ fontSize: 'clamp(0.875rem, 4vw, 1rem)', color: 'rgba(255,255,255,0.4)', lineHeight: 1.5, maxWidth: 380 }}>
              Join hundreds of interns on MR Techlab's unified platform — track tasks, get AI-powered mentorship, and earn verified certificates.
            </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '.625rem', marginTop: '2rem' }}>
              {[
                ...
                <div key={i} className="feature-item" style={{
                  display: 'flex', alignItems: 'center', gap: '.75rem', padding: '.5rem .875rem', borderRadius: 12,
                  background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)',
                  opacity: mounted ? 1 : 0, transform: mounted ? 'none' : 'translateX(-16px)',
                  transition: `all .6s cubic-bezier(.4,0,.2,1) ${0.2 + i * 0.1}s`, cursor: 'default',
                }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: f.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ fontSize: '1rem' }}>{f.icon}</span>
                  </div>
                  <span style={{ fontSize: 'clamp(0.75rem, 3.5vw, 0.8125rem)', color: 'rgba(255,255,255,0.5)' }}>{f.label}</span>
                </div>
              ))}
            </div>

            {/* Stats strip – responsive row or column */}
            <div className="stats-row" style={{
              display: 'flex', justifyContent: 'space-between', gap: '1rem',
              marginTop: '2.5rem', paddingTop: '2rem', borderTop: '1px solid rgba(255,255,255,0.06)',
              flexWrap: 'wrap',
            }}>
              {[
                { val: `${c1}+`, lbl: 'Active Interns' },
                ...
                <div key={i} style={{ flex: 1, minWidth: '80px', textAlign: 'center' }}>
                  <div style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 800, fontSize: 'clamp(1.25rem, 5vw, 1.5rem)', background: 'linear-gradient(135deg,#a78bfa,#67e8f9)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                    {s.val}
                  </div>
                  <div style={{ fontSize: '.7rem', color: 'rgba(255,255,255,0.3)', letterSpacing: '.08em', textTransform: 'uppercase', marginTop: '.125rem' }}>{s.lbl}</div>
                </div>
        ...
        <div style={{
          background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
          padding: 'clamp(1rem, 5vw, 2rem)', overflowY: 'auto', boxSizing: 'border-box',
        }}>
          <div style={{
            width: '100%', maxWidth: 480,
            background: '#fff', borderRadius: 28,
            ...
            <div style={{ marginBottom: '1.75rem' }}>
              <div style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 800, fontSize: 'clamp(1.375rem, 6vw, 1.625rem)', color: '#000', letterSpacing: '-.03em', marginBottom: '.3rem' }}>
                Create your account 🚀
              </div>
              <div style={{ fontSize: 'clamp(0.75rem, 3.5vw, 0.875rem)', color: '#555' }}>
                Join MR Techlab — it takes less than a minute
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '.375rem', marginTop: '.875rem' }}>
                {[0, 1, 2].map(i => {
                  ...
                  return (
                    <div key={i} style={{
                      height: 4, borderRadius: 999,
                      width: (i === 0 || active) ? 28 : 8,
                      background: active || i === 0 ? 'linear-gradient(90deg,#7c3aed,#06b6d4)' : '#e0e0e0',
                      boxShadow: (active || i === 0) ? '0 0 8px rgba(124,58,237,.4)' : 'none',
                      transition: 'all .4s ease',
                    }} />
                  );
                ...
                {ROLES.map(r => {
                  ...
                  return (
                    <button key={r.value} type="button" onClick={() => setForm(f => ({ ...f, role: r.value }))} style={{
                      ...
                      padding: '.875rem 1rem', borderRadius: 14, border: `1.5px solid ${active ? '#7c3aed' : '#e0e0e0'}`,
                      background: active ? 'linear-gradient(135deg,rgba(124,58,237,.07),rgba(6,182,212,.04))' : '#fff',
                      cursor: 'pointer', transition: 'all .2s ease', textAlign: 'left', fontFamily: "'DM Sans', sans-serif",
                      position: 'relative', overflow: 'hidden', boxShadow: active ? '0 0 0 3px rgba(124,58,237,.12)' : 'none',
                    }}>
                      <div style={{
                        ...
                        opacity: active ? 1 : 0, transform: active ? 'scale(1)' : 'scale(.4)', transition: 'all .25s cubic-bezier(.34,1.56,.64,1)',
                      }}>
                        <svg width="10" height="10" fill="none" stroke="#fff" strokeWidth="2.5" viewBox="0 0 12 12"><path d="M2 6l3 3 5-5" /></svg>
                      </div>
                      <span style={{ fontSize: '1.125rem' }}>{r.emoji}</span>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="text" value={form.name} onChange={setField('name')} onFocus={() => setFocused('name')} onBlur={() => setFocused('')}
                    placeholder="Jane Doe" required style={inputStyle('name')} />
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="email" value={form.email} onChange={setField('email')} onFocus={() => setFocused('email')} onBlur={() => setFocused('')}
                    placeholder="you@example.com" required style={inputStyle('email')} />
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type={show ? 'text' : 'password'} value={form.password} onChange={setField('password')}
                    onFocus={() => setFocused('pw')} onBlur={() => setFocused('')}
                    placeholder="Min. 6 characters" required
                    style={{ ...inputStyle('pw'), paddingRight: '2.75rem' }} />
                  <button type="button" onClick={() => setShow(s => !s)} style={{
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="tel" value={form.phone} onChange={setField('phone')} onFocus={() => setFocused('phone')} onBlur={() => setFocused('')}
                    placeholder="+91 98765 43210" style={inputStyle('phone')} />
                </div>
              </div>

              <button type="submit" disabled={loading} style={{
                marginTop: '.25rem', width: '100%', padding: 'clamp(0.75rem, 4vw, 0.875rem)', borderRadius: 14, border: 'none',
                background: loading ? '#ccc' : 'linear-gradient(135deg,#7c3aed 0%,#5b21b6 50%,#312e81 100%)',
                color: '#fff', fontSize: 'clamp(0.875rem, 4vw, 1rem)', fontWeight: 700, fontFamily: "'DM Sans', sans-serif",
                cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '.625rem',
                transition: 'all .25s ease', boxShadow: loading ? 'none' : '0 4px 20px rgba(124,58,237,.4)',
                boxSizing: 'border-box', position: 'relative', overflow: 'hidden',
              }} onMouseEnter={e => { if (!loading) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(124,58,237,.55)'; } }}
                 onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = loading ? 'none' : '0 4px 20px rgba(124,58,237,.4)'; }}>
                {loading ? (
